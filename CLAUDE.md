...
- 11 migrations aplicadas (incluindo `notificacoes_outbound` deferred pra integração futura)
- Tabelas: `conversas`, `mensagens`, `mensagens_recebidas`, `empresas_cache`, `funcionarios_cache`, `agendas_config`, `slots_config`, `agendamentos`, `notificacoes_pendentes`, `notificacoes_outbound`
...
7. **Workflows desativados por padrão:** ativar via UI ou MCP `activateWorkflow` antes de smoke test.
...
2. Seed Supabase: 1 row em `empresas_cache` + `agendas_config` + N rows `slots_config`
3. Ativar workflows: WF1, WF2, WF3, WF4 (WF5 já ativa via cron quando precisar)
4. Configurar webhook Meta Console com URL ngrok atual
5. Smoke test ponta-a-ponta: mandar WhatsApp de número no allowlist
