...
**Transfere pra humano:** qualquer outro tipo de exame, funcionário não encontrado (periodico/demissional), **setor/cargo do funcionário divergentes do cadastro SOC** (periodico/demissional, motivo `dados_funcionario_divergentes`), empresa não cadastrada, hierarquia não encontrada (admissional), erro de cadastro/grave do SOC. Handoff dentro do mesmo número: `conversa.status='transferido'`, bot para de responder, notificação P0 criada.
...
4. **PERIODICO/DEMISSIONAL:** CPF do funcionário → `buscar_funcionario` (retorna `nome/setor/cargo` do SOC via Exporta Dados 192399). Bot **confirma setor+cargo com o cliente antes de pedir data**: cliente diz que ambos estão corretos → segue; setor/cargo errado ou ausente → `transferir_humano` motivo=`dados_funcionario_divergentes` (texto avisa que vai passar pra equipe ajustar cadastro)
...
27b. **Env do exporta funcionário (setor/cargo):** `SOC_EXPORTA_FUNCIONARIO_CODIGO=192399` (Cadastro de Funcionarios Por Empresa) + `SOC_EXPORTA_FUNCIONARIO_CHAVE` no `.env`. Usado por `buscar_funcionario` (WF4 nós `BF - Return Cache` E `BF - Return Found`) pra trazer setor/cargo e confirmar com o cliente. **`empresaTrabalho`=código da empresa cliente é OBRIGATÓRIO** no parâmetro (sem ele → 0 rows; diferente do 191874 que usa só `empresa`). Resposta latin1 (gotcha 20). Como toda env nova, exige **restart** do n8n.
